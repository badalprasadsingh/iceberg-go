...
Each backend package links only its own cloud SDK, so importing `io/gocloud/s3` does not pull in the Google Cloud or Azure SDKs. Importing `io/gocloud` registers all three at once and links all three SDKs.
...
`io.Register` panics on `nil` factory or duplicate scheme. Built-in schemes: `file`, `""` (the empty scheme). Cloud schemes (`s3`, `gs`, `abfs`, etc.) are registered only when the matching backend package under `io/gocloud` is blank-imported.
