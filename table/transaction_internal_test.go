...
// refreshingConflictCatalog rejects every CommitTable with ErrCommitFailed and
// serves post-race metadata on refresh, so a test can drive exactly one retry
// and inspect what the replay does with the refreshed state.
type refreshingConflictCatalog struct {
	refreshed Metadata
	fs        iceio.IO
	attempts  atomic.Int32
}

func (c *refreshingConflictCatalog) LoadTable(_ context.Context, ident Identifier) (*Table, error) {
	return New(ident, c.refreshed, "metadata.json",
		func(context.Context) (iceio.IO, error) { return c.fs, nil }, c), nil
}

func (c *refreshingConflictCatalog) CommitTable(_ context.Context, _ Identifier, _ []Requirement, _ []Update) (Metadata, string, error) {
	c.attempts.Add(1)

	return nil, "", fmt.Errorf("REST: %w", ErrCommitFailed)
}
...
func TestBranchCommitRejectsNameThatBecomesATagOnRetry(t *testing.T) {
	...
	cat := &refreshingConflictCatalog{refreshed: tagMeta, fs: memIO}
